...
export function createAssetRepository(database: AppDatabase) {
  ...
  const listAll = database.prepare('SELECT id,product_id as productId,filename,stored_name as storedName,mime_type as mimeType,size,created_at as createdAt FROM product_assets ORDER BY created_at')
  return {
    ...
    listAll: () => listAll.all() as ProductAsset[],
    replaceAll(assets: ProductAsset[]) {
      database.exec('BEGIN')
      try {
        database.exec('DELETE FROM product_assets')
        for (const asset of assets) insert.run(asset.id, asset.productId, asset.filename, asset.storedName, asset.mimeType, asset.size, asset.createdAt)
