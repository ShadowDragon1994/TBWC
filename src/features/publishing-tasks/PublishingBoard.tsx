...
export function PublishingBoard({ onNotice }: { onNotice: (message: string) => void }) {
  ...
  const autoPublish = async (task: PublishingTask) => {
    setPublishingId(task.id)
    try {
      const result = await publishingTasksApi.autoPublish(task.id)
      setTasks(current => current.map(item => item.id === task.id ? result.data.task : item))
      onNotice('小红书自动发布完成，执行记录已写入审计')
    } catch (error) {
      onNotice(error instanceof Error ? error.message : '自动发布失败')
    } finally {
