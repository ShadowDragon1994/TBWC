...
export function ProductLibrary({ onUse }: { onUse: (product: ProductRecord) => void }) {
  ...
  const [sourcingSimulated, setSourcingSimulated] = useState<boolean | null>(null)
  ...
  const searchSourcing = async () => {
    if (!sourcingQuery.trim()) { setMessage('请输入1688货源关键词'); return }
    setSourcingBusy(true)
    try { const result = await productsApi.sourcing1688(sourcingQuery); setSourcingOffers(result.data); setSourcingSimulated(result.meta.simulated); setSourcingOpen(true) }
    catch (error) { setMessage(error instanceof Error ? error.message : '1688货源采集失败') }
    finally { setSourcingBusy(false) }
  }
  const importSourcing = async (offer: SourcingOffer) => {
    setImportingId(offer.id)
    try { await productsApi.import1688(offer.id); setMessage(`已从1688货源导入：${offer.title}`); await load() }
  ...
  return <section className="library-page">
    <div className="library-head"><div><h1>商品库</h1><p>商品资料和图片保存在这台电脑上。</p></div><div className="library-actions"><button onClick={() => setSourcingOpen(true)}><PackagePlus size={16}/>1688货源采集</button><button onClick={() => void downloadBackup()}><Download size={16}/>导出完整备份</button><label className="button"><Upload size={16}/>恢复完整备份<input type="file" accept="application/zip,.zip" onChange={event => void restoreBackup(event.target.files?.[0])}/></label><button className="primary" onClick={() => edit()}><Plus size={17}/>新增商品</button></div></div>
    ...
    {sourcingOpen && <div className="product-dialog-backdrop" role="presentation"><section className="product-dialog sourcing-dialog" role="dialog" aria-modal="true" aria-label="1688货源采集"><header><div><h2>1688货源采集</h2><p>{sourcingSimulated === null ? '输入商品方向，通过 Edge RPA 搜索1688。首次使用请在弹出的浏览器中登录。' : sourcingSimulated ? '当前为模拟回退数据。' : '当前结果来自 Edge RPA，已保存到本地货源池。'}</p></div><button onClick={() => setSourcingOpen(false)}>×</button></header><div className="sourcing-search"><input aria-label="搜索1688货源" value={sourcingQuery} onChange={event => setSourcingQuery(event.target.value)} placeholder="例如：七夕定制香囊礼盒"/><button disabled={sourcingBusy} onClick={() => void searchSourcing()}><Search size={16}/>{sourcingBusy ? '采集中…' : '搜索'}</button></div><div className="sourcing-list">{sourcingOffers.length === 0 ? <p>输入关键词后开始采集货源</p> : sourcingOffers.map(offer => <article key={offer.id}><div><small>{offer.category} · 起订 {offer.minOrder} 件</small><h3>{offer.title}</h3><p>{offer.sellingPoints}</p><span>供货价 ¥{offer.wholesalePrice.toFixed(2)} · 建议零售价 ¥{offer.suggestedRetailPrice.toFixed(2)}</span></div><button disabled={importingId === offer.id} onClick={() => void importSourcing(offer)}>{importingId === offer.id ? '导入中…' : '导入商品库'}</button></article>)}</div></section></div>}
