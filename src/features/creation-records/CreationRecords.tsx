...
import { creationRecordsApi, type CreationRecord, type CreationRecordFilters, type CreationRecordSource } from './creation-records.api'
...
export function CreationRecords({ onContinue }: { onContinue: (record: CreationRecord) => void }) {
  ...
  const load = useCallback(async (filters: CreationRecordFilters) => {
    try {
  ...
  useEffect(() => { void load({ q: '', productName: '', platform: '', source: '' }) }, [load])
  ...
  const remove = async (record: CreationRecord) => {
    ...
    setMessage('创作记录已删除')
    await load({ q: query, productName, platform, source })
  }
