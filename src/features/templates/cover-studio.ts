...
export async function exportCoverPng(options: { platform: ContentPlatform; title: string; subtitle: string; preset: BrandPreset; template: number; imageUrl: string }) {
  ...
  const imageSize = Math.min(spec.width * .72, spec.height * .44); context.drawImage(image, (spec.width - imageSize) / 2, spec.height * .16, imageSize, imageSize)
