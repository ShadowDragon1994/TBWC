...
export const performanceApi={
  list:()=>api<{data:PerformanceRecord[]}>('/api/performance-records?'),
  create:(draft:PerformanceDraft)=>api<{data:PerformanceRecord}>('/api/performance-records',json('POST',draft)),
  importMany:(records:PerformanceDraft[])=>api<{data:{created:number;skipped:number;records:PerformanceRecord[]}}>('/api/performance-records/import',json('POST',{records})),
  remove:(id:string)=>api<void>(`/api/performance-records/${id}`,{method:'DELETE'}),
}
