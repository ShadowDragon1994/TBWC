...
export type AiGenerateDraft = { platform: ContentPlatform; product: { name: string; category: string; price: number; material?: string; audience?: string; scene?: string; sellingPoints?: string; forbiddenTerms?: string }; guidance: string }
...
export const aiApi = {
  ...
  generate: (draft: AiGenerateDraft) => api<{ data: AiGeneratedContent }>('/api/ai/generate', json('POST', draft)),
}
